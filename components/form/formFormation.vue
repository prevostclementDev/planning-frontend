...
async function submit(){
  ...
  if (  !v$.value.$error ) {
    ...
    const idToast = $toast.loading('Création de la formation...')
    ...
    if ( ! useFetchStore.state.error[url] ) {
      ...
      $toast.update(idToast, {
        render: 'Création de la formation réussi',
        autoClose: true,
        type: 'success',
        isLoading: false,
      });
      ...
      $toast.update(idToast, {
        render: 'Une erreur est survenue pendant la création de la formation',
        autoClose: true,
        type: 'error',
        isLoading: false,
      });
...
<template>
  <form action="" @submit.prevent="submit">
      ...
      <Input
        placeholder="Développement fullstack 3éme année"
        title="Le nom de votre formation"
